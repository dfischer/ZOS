...
typedef struct fat32_nodee {
    ...
    struct fat32_nodee* parent; // pointer to another fs_node
...
fat32_node* readdir_fat32(partition* cpart, vbr32_t* vbr, fat32_node* directory) {
    ...
    while (current_cluster) {
        unsigned int sector = (current_cluster-2)*vbr->sectors_per_cluster + first_data_sector; // The actual sector, offset from the start of the partition

        memset(entries, 0, sizeof(dir_entry));
        unsigned char err = ide_read_sectors(cpart->drive, 8, cpart->sector + sector, 0, (unsigned short *)entries);
        if (err > 0) {ide_print_error(cpart->drive, err); return 0;}
...
void init_fat32(unsigned char device, unsigned char pnum) {
    partition* cpart = get_partition(device, pnum);
    vbr32_t* vbr = read_vbr(cpart->drive, cpart->sector);

    //show_info(vbr);

    fat32_node* root_node = get_rn_fat32(vbr);
